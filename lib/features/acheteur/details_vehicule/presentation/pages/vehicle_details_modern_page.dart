...
class VehicleDetailsModernPage extends ConsumerStatefulWidget {
  ...
  const VehicleDetailsModernPage({
    super.key,
    this.annonceId,
    this.details,
  }) : assert(annonceId != null || details != null, 
             'Either annonceId or details must be provided');

  @override
  ConsumerState<VehicleDetailsModernPage> createState() => _VehicleDetailsModernPageState();
}

class _VehicleDetailsModernPageState extends ConsumerState<VehicleDetailsModernPage> {
  @override
  void initState() {
    super.initState();
  ...
  Widget _buildContent(VehicleDetailsModel details) {
    ...
    return CustomScrollView(
      slivers: [
        // Header avec image
        SliverToBoxAdapter(
          child: _buildImageHeader(details, isFavorite),
        ),
    ...
    return Stack(
      children: [
        ...
        Container(
          ...
          child: photoUrl == null
              ? const Icon(Icons.directions_car, size: 100, color: AppColors.textLight)
              : null,
        ),
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: equipments.map((e) => Container(
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          const Icon(Icons.star, color: AppColors.starFilled, size: 16),
                          const SizedBox(width: 4),
                          Text(
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          ElevatedButton(
            onPressed: widget.annonceId != null 
                ? () => ref.read(detailsProvider(widget.annonceId!).notifier).loadDetails()
                : null,
  ...
  void _shareAnnonce(VehicleDetailsModel details) {
    final vehicle = details.annonce.vehicle;
    final shareText = '🚗 ${vehicle.make} ${vehicle.model} ${vehicle.year}\n'
        '💰 ${_formatPrice(details.annonce.price)}\n'
                ...
                _buildShareOption(
                  ...
                  onTap: () {
                    Clipboard.setData(ClipboardData(
                      text: 'https://occazcar.fr/annonce/${details.annonce.id}',
                    ));
                    Navigator.pop(context);
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
  ...
  Future<void> _contactSeller(VehicleDetailsModel details) async {
    ...
    final annonceTitre = '${details.annonce.vehicle.make} ${details.annonce.vehicle.model}';
    ...
    try {
      ...
      final conversation = await ref.read(conversationsProvider.notifier).startConversation(
        sellerId: sellerId,
        annonceId: annonceId,
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) => ChatPage(
            ...
            peerId: sellerId,
            currentUserId: ref.read(conversationsProvider.notifier).currentUserId ?? 'user',
            peerName: sellerName,
          ),
        ),
      );
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              Navigator.pop(context);
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Numéro copié !'), backgroundColor: AppColors.success),
              );
            },
            style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary),
          ...
          children: [
            ...
            Expanded(
              child: ListView(
                padding: const EdgeInsets.all(20),
                children: [
                  _buildSpecRow('Carburant', features.fuelType ?? 'Essence', Icons.local_gas_station),
                  _buildSpecRow('Puissance', '${features.horsePower ?? 130} CV', Icons.speed),
                  _buildSpecRow('Boîte de vitesses', features.transmission ?? 'Manuelle', Icons.settings),
                  _buildSpecRow('Portes', '${features.doors ?? 5} portes', Icons.door_front_door),
                  _buildSpecRow('Places', '${features.seats ?? 5} places', Icons.event_seat),
                  _buildSpecRow('Couleur', features.color ?? 'Non spécifié', Icons.palette),
                  _buildSpecRow('Énergie', features.energy ?? 'C', Icons.eco),
                  _buildSpecRow('Climatisation', features.hasAirConditioning ? 'Oui' : 'Non', Icons.ac_unit),
                  _buildSpecRow('GPS', features.hasGPS ? 'Oui' : 'Non', Icons.gps_fixed),
                  _buildSpecRow('Radar de recul', features.hasParkingSensors ? 'Oui' : 'Non', Icons.sensors),
                  _buildSpecRow('Bluetooth', features.hasBluetoothPhone ? 'Oui' : 'Non', Icons.bluetooth),
                  ...
                  if (features.otherFeatures.isNotEmpty) ...[
                    ...
                    Wrap(
                      spacing: 8,
                      runSpacing: 8,
                      children: features.otherFeatures.map((feature) => 
                        Chip(
                          label: Text(feature),
                          backgroundColor: AppColors.primary.withAlpha((0.1 * 255).round()),
